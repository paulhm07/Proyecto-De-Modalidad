...
export class PadresService {
  ...
  // Lista hijos vinculados a un padre (solo verificados y activos)
  ...
  // Solicitar vínculo por nombre + PIN del hijo
  async solicitarVinculo(padreId: string, data: { nombre: string; pin: string; parentesco?: string }) {
    const padre = await this.prisma.usuario.findFirst({ where: { id: padreId, rol: 'PADRE' } });
    if (!padre) throw new BadRequestException('Padre no encontrado');

    const hijo = await this.prisma.usuario.findFirst({
      where: { nombre: data.nombre, pin: data.pin, rol: 'ESTUDIANTE' },
    });
    if (!hijo) throw new NotFoundException('No encontramos un estudiante con ese nombre y PIN');
    ...
    if (existente) {
      ...
      throw new BadRequestException('Ya tienes vinculado a este estudiante');
    }
  ...
  async vincularPorHijoId(padreId: string, hijoId: string, parentesco?: string) {
    const padre = await this.prisma.usuario.findFirst({ where: { id: padreId, rol: 'PADRE' } });
    if (!padre) throw new BadRequestException('Padre no encontrado');
    const hijo = await this.prisma.usuario.findFirst({ where: { id: hijoId, rol: 'ESTUDIANTE' } });
    if (!hijo) throw new NotFoundException('Estudiante no encontrado');
  ...
  async verificarAccesoHijo(padreId: string, hijoId: string) {
    const vinculo = await this.prisma.padreHijo.findUnique({
      where: { padreId_hijoId: { padreId, hijoId } },
    });
  ...
  async obtenerResumen(padreId: string, hijoId: string) {
    ...
    const porcentajeAsistencia =
      totalAsistencias > 0 ? (presentes / totalAsistencias) * 100 : 100;
  ...
  async obtenerCalificaciones(
    ...
    const where: any = { estudianteId: hijoId };
    if (opts?.desde || opts?.hasta) {
      where.calificadaEn = {};
      if (opts?.desde) where.calificadaEn.gte = new Date(opts.desde);
      if (opts?.hasta) where.calificadaEn.lte = new Date(opts.hasta);
    }
    ...
    const porAsignatura: Record<string, { nombre: string; notas: number[] }> = {};
    ...
    const promedioGeneral =
      todasNotas.length > 0 ? todasNotas.reduce((s, n) => s + n, 0) / todasNotas.length : 0;
  ...
  async obtenerAvisos(padreId: string, opts?: { tipo?: string; soloNoLeidos?: boolean }) {
    const vinculos = await this.prisma.padreHijo.findMany({
      where: { padreId, activo: true, verificado: true },
      select: { hijoId: true },
    });
    ...
    const where: any = { seccionId: { in: seccionIds }, activo: true };
  ...
  async marcarAvisoLeido(padreId: string, avisoId: string) {
  ...
  async firmarAviso(padreId: string, avisoId: string) {
    const aviso = await this.prisma.aviso.findUnique({ where: { id: avisoId } });
    if (!aviso) throw new NotFoundException('Aviso no encontrado');
    if (!aviso.requiereFirma) {
      throw new BadRequestException('Este aviso no requiere firma');
    }
  ...
  async obtenerConversaciones(padreId: string) {
  ...
  async obtenerConversacion(padreId: string, conversacionId: string) {
    ...
    if (conv.padreId !== padreId) throw new ForbiddenException('Sin acceso a esta conversación');
  ...
  async enviarMensaje(padreId: string, conversacionId: string, cuerpo: string) {
    const conv = await this.prisma.conversacion.findUnique({
      where: { id: conversacionId },
    });
    if (!conv) throw new NotFoundException('Conversación no encontrada');
    if (conv.padreId !== padreId) throw new ForbiddenException('Sin acceso a esta conversación');
    if (!cuerpo?.trim()) throw new BadRequestException('El mensaje no puede estar vacío');
  ...
  async iniciarConversacion(
    ...
    if (seccionesMaestro.length === 0) {
      throw new BadRequestException('Este maestro no enseña a tu hijo/a');
    }
  ...
  async obtenerNotificaciones(padreId: string, opts?: { soloNoLeidos?: boolean }) {
    const where: any = { usuarioId: padreId };
    if (opts?.soloNoLeidos) where.leida = false;
  ...
  async marcarNotificacionLeida(padreId: string, notificacionId: string) {
    const notif = await this.prisma.notificacion.findUnique({
      where: { id: notificacionId },
    });
    if (!notif) throw new NotFoundException('Notificación no encontrada');
    if (notif.usuarioId !== padreId) throw new ForbiddenException('Sin acceso');
  ...
  async marcarTodasLeidas(padreId: string) {
  ...
  async suscribirPush(
    padreId: string,
    data: { endpoint: string; p256dh: string; auth: string; dispositivo?: string }
  ) {
  ...
  async desuscribirPush(padreId: string, endpoint: string) {
    const sub = await this.prisma.pushSubscription.findUnique({
      where: { endpoint },
    });
    if (!sub) return { ok: true };
    if (sub.usuarioId !== padreId) throw new ForbiddenException('Sin acceso');
    return this.prisma.pushSubscription.update({
      where: { id: sub.id },
      data: { activa: false },
    });
  ...
  // ===================== TAREAS Y MEDALLAS DEL HIJO =====================

  async obtenerTareasHijo(padreId: string, hijoId: string, opts?: { estado?: string }) {
    await this.verificarAccesoHijo(padreId, hijoId);
    ...
    const where: any = { seccionId: { in: seccionIds } };
  ...
  async obtenerMedallasHijo(padreId: string, hijoId: string) {
    await this.verificarAccesoHijo(padreId, hijoId);
  ...
  async seedPadreDemo(padreId: string) {
    const padre = await this.prisma.usuario.findFirst({ where: { id: padreId, rol: 'PADRE' } });
    if (!padre) throw new BadRequestException('Padre no encontrado');
    ...
    if (!hijo) throw new NotFoundException('DemoKid no encontrado. Inicia sesión como estudiante primero.');
    ...
    if (!vinculoExistente) {
      ...
      // Si existe pero no estaba verificado (migración de schema), lo verificamos
          ...
          data: [
            {
              ...
              contenido:
                'Estimados padres, les invitamos a la reunión general del jueves 15 de agosto a las 5:00 PM. Hablaremos sobre el progreso académico del bimestre.',
              prioridad: 3,
            ...
            },
            {
              ...
              contenido:
                'Las boletas del segundo bimestre se entregarán el viernes. Favor de pasar a recogerlas en horario de 8:00 AM a 12:00 MD.',
              prioridad: 2,
            },
            {
              seccionId: seccion.id,
              ...
              contenido:
                'El viernes 23 de agosto celebraremos el Día del Estudiante. Los niños pueden venir con ropa deportiva.',
              prioridad: 1,
      ...
      if (!convExistente) {
        ...
        await this.prisma.mensaje.create({
          data: {
            ...
            cuerpo:
              'Hola, queríamos comentarle que DemoKid ha mejorado mucho en fracciones esta semana. ¡Felicitaciones por el apoyo en casa!',
