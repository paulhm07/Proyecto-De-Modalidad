...
// Valores que antes provenían del enum TipoDesafio (SQLite no soporta enums)
const TIPO_SELECCION_MULTIPLE = 'SELECCION_MULTIPLE';
const TIPO_COMPLETAR_TEXTO = 'COMPLETAR_TEXTO';

// Tipos adicionales (modelados como selección múltiple en el frontend actual,
// pero con etiqueta descriptiva para que el estudiante sepa qué hacer).
const TIPO_VERDADERO_FALSO = 'VERDADERO_FALSO';
const TIPO_ASOCIAR = 'ASOCIAR_PAREJAS';
const TIPO_ORDENAR = 'ORDENAR_PALABRAS';

interface DesafioSeed {
  tipo: string;
  pregunta: string;
  puntos: number;
...
export class DesafiosService {
  ...
  async crearAsignatura(nombre: string, descripcion: string | null, maestroId: string) {
  ...
  async actualizarAsignatura(id: string, data: { nombre?: string; descripcion?: string | null }) {
    const asig = await this.prisma.asignatura.findUnique({ where: { id } });
    if (!asig) throw new NotFoundException('Asignatura no encontrada');
  ...
  async crearModulo(asignaturaId: string, titulo: string, nivelMinimo: number = 1, descripcion?: string) {
  ...
  async actualizarModulo(id: string, data: { titulo?: string; descripcion?: string | null; nivelMinimo?: number }) {
  ...
  async crearDesafio(data: {
    ...
    if (!tiposValidos.includes(data.tipo)) {
      throw new BadRequestException(`Tipo de desafío inválido. Debe ser uno de: ${tiposValidos.join(', ')}`);
    }

    if (!data.opciones || data.opciones.length < 2) {
      throw new BadRequestException('Debe incluir al menos 2 opciones');
    }
  ...
  async eliminarDesafio(id: string) {
    ...
    if (!d) throw new NotFoundException('Desafío no encontrado');
  ...
  private buildCurriculumData(): AsignaturaSeed[] {
    return [
      {
        nombre: 'Matemáticas',
  ...
  private buildMedallasData(asignaturas: AsignaturaSeed[]): MedallaSeed[] {
    const medallas: MedallaSeed[] = [];
  ...
  private tituloMedallaPara(tituloModulo: string): string {
