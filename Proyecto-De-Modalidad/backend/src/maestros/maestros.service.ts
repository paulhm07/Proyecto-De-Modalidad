...
export class MaestrosService {
  ...
  async crearSeccion(data: {
    ...
    if (!maestro) throw new BadRequestException('Maestro no encontrado');
  ...
  async obtenerSeccionesDeMaestro(maestroId: string) {
  ...
  async obtenerSeccion(id: string) {
  ...
  async actualizarSeccion(
    id: string,
    data: { nombre?: string; grado?: number; activa?: boolean; asignaturaId?: string | null },
  ) {
  ...
  async inscribirEstudiante(seccionId: string, estudianteId: string) {
    ...
    if (existente) {
      if (existente.activa) return { mensaje: 'Ya inscrito', inscripcion: existente };
  ...
  async desinscribirEstudiante(seccionId: string, estudianteId: string) {
  ...
  async obtenerEstudiantesDeSeccion(seccionId: string) {
  ...
  async obtenerTareasDeSeccion(seccionId: string) {
  ...
  async actualizarTarea(
    id: string,
    data: { titulo?: string; descripcion?: string | null; fechaLimite?: string; estado?: string },
  ) {
    ...
    if (data.fechaLimite !== undefined) {
      ...
      if (isNaN(fecha.getTime())) throw new BadRequestException('fechaLimite inválida');
      updateData.fechaLimite = fecha;
    }
    if (data.estado !== undefined) {
      if (!ESTADOS_TAREA.includes(data.estado)) {
        throw new BadRequestException(`Estado inválido. Debe ser uno de: ${ESTADOS_TAREA.join(', ')}`);
      }
  ...
  async obtenerCalificacionesDeTarea(tareaId: string) {
  ...
  async obtenerCalificacionesDeEstudiante(estudianteId: string) {
    return this.prisma.calificacion.findMany({
      where: { estudianteId },
  ...
  async registrarAsistenciaBatch(
    ...
    for (const r of registros) {
      if (!ESTADOS_ASISTENCIA.includes(r.estado)) {
        throw new BadRequestException(`Estado inválido: ${r.estado}`);
      }
      const fecha = new Date(r.fecha);
      if (isNaN(fecha.getTime())) throw new BadRequestException(`Fecha inválida: ${r.fecha}`);
  ...
  async obtenerAsistenciaDeSeccion(seccionId: string, fecha?: string) {
    const where: any = { seccionId };
    if (fecha) {
      const f = new Date(fecha);
      if (!isNaN(f.getTime())) where.fecha = f;
    }
  ...
  async obtenerAsistenciaDeEstudiante(estudianteId: string) {
  ...
  async actualizarAsistencia(id: string, data: { estado?: string; observacion?: string | null }) {
    if (data.estado && !ESTADOS_ASISTENCIA.includes(data.estado)) {
      throw new BadRequestException(`Estado inválido. Debe ser uno de: ${ESTADOS_ASISTENCIA.join(', ')}`);
    }
  ...
  async resumenSeccion(seccionId: string) {
    const seccion = await this.prisma.seccion.findUnique({
      where: { id: seccionId },
      include: {
  ...
  async reporteEstudiante(estudianteId: string) {
  ...
  async alertasMaestro(maestroId: string) {
  ...
  // ===================== SEED DEMO =====================
  // Crea una sección demo para MaestroDemo con DemoKid + estudiantes de prueba
  async seedDemo(maestroId: string) {
    const maestro = await this.prisma.usuario.findFirst({
      where: { id: maestroId, rol: 'MAESTRO' },
    });
    if (!maestro) throw new NotFoundException('Maestro no encontrado');

    // Buscar o crear una sección
    ...
    // Inscribir a todos en la sección
